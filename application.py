...
genotype_cols = list(genotypes.columns)
...
merge_tab = [
    dbc.Container([
        ...
        dbc.Col(html.H3("Raw Trait Distribution")),
        dbc.Col(html.H5("For many statistical analysis, it’s often assumed that the trait of interest (the dependent variable) is normally distributed. This assumption if often broken due to multiple factors. In cases where this assumption is violated, extreme values can be overrepresented, and in somecases the conclusions of the model might be misleading (see: Simpson Paradox). To address theses issues our pipeline performs a quantile normalization, which is a transformation that preserves the rank of each subject but reassigns values such that the final distribution is perfectly normally distributed. This means that our GWAS identifies biallelic snps that contribute to a change in the rank of the phenotype (e.g. having a mutation on snp 1:3029384 contributes to a rat to go from the top 70% percentile of the trait to the top 75%).")),
        dbc.Col(html.H3("Covariates Regressed Out")),
        dbc.Col([html.H5("Covariates may also influence a trait but are not of interest, so should be removed. Common examples include “age”, “weight”, “coat color”, “cohort”, and “phenotyping center”. We work with PI to determine which covariates should be considered. In order to “regress out” the part of the phenotype variance that is related to known covariates, we follow the procedure of fitting a linear model that predicts the desired trait based solemnly on the measured covariates. Then the trait is subtracted by the trait predictions generated by the linear model described above. The resulting subtraction is expected to be independent of the covariates as all the effects caused by those covaraites were removed. Since this method utilizes a linear regression to remove those effect, non-linear effects of those covariates onto the traits will not be addressed and assumed to be null. Categorical traits (such as sex and cohort) are in order to convert them into a format that can be used on a linear regression, with this particular encoding assuming that each category is equally unique and unrelated to each other. In certain cases, it’s possible that accounting for too many covariates might ‘overcorrect’ the trait. To address this issue we are ‘regress out’ only the covariates that explain more the 2% of the variance of the trait. This calculation is often called r^2 and is estimated as cov(covariant, trait)/variance(trait). Lastly, the corrected trait is quantile normalized again, as it’s expected to follow a normal distribution.")]),
...
GWAS_tab=[ dbc.Row([
           ...
           dbc.Col([dcc.Loading(id="loading gwas",type="default", children= dcc.Tabs([
               ...
               dcc.Tab(label = 'QTLs', children = html.Div([html.H5('Quantitative trait loci (QTLs) are regions in the genome that contain single nucleotide polymorphisms (SNPs) that correlate with a complex trait.'), dcc.Input(id="igvmanual", type="text", placeholder="manually query SNP",  debounce=True), \
               ...
               dcc.Tab(label = 'eQTLs', children = html.Div([html.Div(id = 'eqtl_table',className="dbc-row-selectable")],
                                                                style = {'height': '1000px', 'width' : '1500px','margin-left':'30px'},
                                                                id='eqtldiv' ) ),
               dcc.Tab(label = 'pheWAS', children = html.Div([
                   html.H3('Phenotype Wide Association Study (PheWAS):'),
                   html.H5('This table report the correlation between the topSNP and traits from other studies in HS rats conducted by the center. Use information from these tables to better understand what additional phenotypes this interval may be associated with. If the column "exact match" is true, the same snp was detected by the GWAS of other traits. When the column "exact match" is False, we detected a highly correlated SNP (>.8 r2) within a 3Mb window. For a project-trait combination only the top SNP within the 3Mb window is shown'),\
                   html.Div(id = 'phewas_table',className="dbc-row-selectable")],
                                                                style = {'height': '1000px', 'width' : '1500px','margin-left':'30px'},
                                                                id='phediv' ) ),
...
def preprocess_data(project_name):
    ...
    html_var = urlreq.urlopen(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{project_name}/data_distributions.html').read()    
    ...
    qtl_table = dash_table.DataTable(data=igv_options.to_dict('records'), columns=[{'name': str(i), 'id': str(i),
                                     ...
                                     style_table={'overflowX': 'auto'},
                                                      style_cell={'minWidth': '180px', 'width': '90px', 'maxWidth': '180px','overflow': 'hidden','textOverflow': 'ellipsis'}, style_as_list_view=True,
                                                      style_header={'backgroundColor': 'rgb(230, 230, 230)','fontWeight': 'bold'} ,
                                    row_selectable="single",   filter_action="native", sort_action="native",  page_size=5, id ='qtl_table_table')
    try:
    ...
    phewas_table = dash_table.DataTable(data=phewas_info.to_dict('records'), columns=[{'name': str(i), 'id': str(i),
                                     ...
                                     style_table={'overflowX': 'auto'},
                                                      style_cell={'minWidth': '180px', 'width': '90px', 'maxWidth': '180px','overflow': 'hidden','textOverflow': 'ellipsis'}, style_as_list_view=True,
                                                      style_header={'backgroundColor': 'rgb(230, 230, 230)','fontWeight': 'bold'} ,
                                    row_selectable="single", filter_action="native", sort_action="native",  page_size=20)
    ...
    heritable = heritable.reset_index(names = 'trait').drop(['LRT', 'df', 'logL0'], axis = 1).rename({"V(G)/Vp": 'heritability'}, axis = 1).sort_values('heritability', ascending = False)
    heritable['Pval'] = -np.log10(heritable['Pval'])
    heritable = heritable.rename({'V(G)': 'Genetic Variation', 'V(e)': 'Environmental Variation', 'Vp': 'Phenotypic Variation'}, axis =1)
...
def update_UMAP_and_ML_select_columns(inpt, data): #, columns_list_id
    ...
    try:
        df = pd.read_json(data)
        #all_genotypes = [{'label': 'genotypes', 'value': 'genotypes'}]
        cols_genotypes = [{'label': x, 'value': x} for x in genotype_cols]
...
def generate_UMAP(clicked, cat_labels, cont_labels,y ,n_nb, radio_val,MLcolumns, MLfuncs, MLclassif, dataframe_json):
    ...
    if dataframe_json != None:
        #cont_labels = flatten_cols(cont_labels)
        df = pd.read_json(dataframe_json)
        if 'genotypes' in cont_labels:
...
def run_fbprophet(click,data, data_umap, y_column, ds_column, regressors, rolling_avg, growth, cap, floor, seasonality, season_mode, season_scale, change_points_n, change_points_prior, change_points_range,
    ...
    if data != None:
        ...
        if df2.shape[1] > df.shape[1]:
            ...
            regressors = [x.replace(' ', '_') for x in [x for x in regressors]]
...
if __name__ == '__main__':
    application.run_server(debug=True, port=8080, threaded=True)#0.0.0.0 host = '0.0.0.0', debug = True,debug = True, ssl_context='adhoc'
