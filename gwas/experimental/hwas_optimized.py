...
def _is_missing_vcf_scalar(x: Any) -> bool:
    """Return True for scalar VCF missing values that pysam may expose."""
    if x is None:
        return True
    if isinstance(x, str):
        return x in {"", ".", "NA", "NaN", "nan"}
    try:
...
def _convert_value(value: Any, *, ncomp: int, dtype, collapse_phased: bool = False):
    """Convert one pysam FORMAT/INFO value to a fixed-width scalar/vector.

    pysam represents missing VCF vector entries as ``None``. Directly casting
    tuples such as ``(0, None, 1)`` to an integer or floating dtype raises a
    ``TypeError``. This converter replaces missing scalar entries with the
    field-specific fill value before casting, so FORMAT fields such as GT/AD/HD
    remain computable even when some samples have partial missing values.
    """
    ...
    if ncomp <= 1:
        ...
        if dt == np.dtype(object):
            return None if _is_missing_vcf_scalar(v0) else str(v0)
        if _is_missing_vcf_scalar(v0):
            return fill
        return np.asarray([v0], dtype=dt)[0]
    ...
    if collapse_phased and len(vals) == 2 * ncomp and len(vals) > 2:
        # Collapse phased vectors by summing corresponding haplotype components.
        # Missing phased components propagate as NaN for numeric probability/dosage fields.
        left = np.asarray([np.nan if _is_missing_vcf_scalar(x) else x for x in vals[:ncomp]], dtype=np.float32)
        right = np.asarray([np.nan if _is_missing_vcf_scalar(x) else x for x in vals[ncomp:]], dtype=np.float32)
        vals = (left + right).tolist()

    vals = vals[:ncomp]
    if dt == np.dtype(object):
        out[:len(vals)] = [None if _is_missing_vcf_scalar(x) else str(x) for x in vals]
    else:
        clean = [fill if _is_missing_vcf_scalar(x) else x for x in vals]
        out[:len(clean)] = np.asarray(clean, dtype=dt)
    return out
...
def _chunk_reader_all_formats(
    ...
    class _FetchFallback(Exception):
        pass
    ...
    try:
        with pysam.VariantFile(bcf_path) as vf:
            ...
            for chrom, grp in grouped:
                ...
                try:
                    iterator = vf.fetch(str(chrom), start, stop)
                except Exception as exc:
                    # Unindexed VCF/BCF or contig-name fetch problems should fall back
                    # to a linear scan. Conversion errors inside _fill_record should
                    # not be swallowed and retried.
                    raise _FetchFallback() from exc
                for rec in iterator:
                    _fill_record(rec)
    except _FetchFallback:
        with pysam.VariantFile(bcf_path) as vf:
            vf.subset_samples(sample_ids)
            for rec in vf:
                _fill_record(rec)
