...
def gencode_to_refflat(gencode_file,out_file,gencode_tag=None):
    ...
    with open(out_file,'w') as out:
        ...
        print("\t".join("geneName name chrom strand txStart txEnd cdsStart cdsEnd exonCount exonStarts exonEnds".split()), file = out );
...
class SQLiteCommand(SQLiteI):
    ...
    def _run_cmd(self,cmds):
        ...
        print("pragma synchronous=OFF;", file = f);
        print("pragma cache_size=500000;", file = f);
        print("pragma page_size=4096;", file = f);
        print("pragma temp_store=MEMORY;", file = f);
