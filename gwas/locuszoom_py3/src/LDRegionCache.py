...
class LDRegionCache():
  def __init__(self,key,dbfile):
    ...
    if proto == None or proto != LD_CACHE_PROTOCOL:
      if existed:
        print("Error: LD cache was created with an older version"\
        " of the program. Please delete '%s', or specify a new cache file, and re-run." % dbfile, file = sys.stderr)
        sys.exit(1)
      else:
    ...
    try:
      ...
      print( "Error: accessing LD cache failed. Please contact admin with following info: ", file = sys.stderr)
      print( "Cache location: %s" % dbfile, file = sys.stderr)
      print( "Available keys: ", file = sys.stderr)
      for key in self.db.keys():
        print( str(key))
      print( "Protocol version: %s" % self.db.get('proto'), file = sys.stderr)
      sys.exit(1)
      
    if self.root == None:
      print( "Creating new LD cache branch for current LD source/population/build..", file = sys.stderr)
