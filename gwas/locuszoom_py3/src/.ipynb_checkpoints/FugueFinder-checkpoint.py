...
class FugueFinder():
  ...
  def write(self,filename):
    try:
      f = open(filename,'w')
      print("snp1 snp2 dprime rsquare", file = f)
      ...
      print("Error: could not write computed LD to disk, permissions?", file = sys.stderr)
      return False
  ...
  def _makeFugueFiles(self,chr,start,end):
    ...
    for line in map:
      ...
      if map_pos <= end and map_pos >= start:
        print("M %s" % map_snp_chrpos, file = dat)
      else:
        print( "S2 %s" % map_snp_chrpos, file = dat)

      # Write out map file. 
      print( "%i %s %i" % (map_chr,map_snp_chrpos,map_pos), file = map_out)
  ...
  def _runNewFugue(self,chr,fixed_dat,fixed_map,ref_snp):
    ...
    if proc.returncode != 0:
      print("Error: new_fugue did not complete successfully. Check logs for more information.", file = sys.stderr)
      sys.exit(1)
