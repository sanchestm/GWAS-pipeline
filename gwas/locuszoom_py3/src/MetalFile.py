...
class MetalFile:
  ...
  def load(self,file,pval_cutoff=None):
    ...
    for line in f:
      ...
      if len(e) != ncols:
        print( "Error: line had %i columns, should have %i:" % (len(e),ncols), file = sys.stderr)
  ...
  def write(self,file=sys.stdout,delim="\t"):
    print(delim.join(['snp','pval','chr','pos']), file = file)
    for line in self.data:
      print(delim.join([str(i) for i in line]), file = file)
