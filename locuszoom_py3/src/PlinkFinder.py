...
class PlinkFinder():
  ...
  def write(self,filename):
    try:
      ...
      print ( "Error: could not write computed LD to disk, permissions?", file = f)
      return False
  ...
  def _run_plink(self):
    ...
    if proc.returncode != 0:
      ...
      if not bMarkerMissing:
        print ("Error: PLINK did not complete successfully. Please check the log file (run with --no-cleanup to see the directory with the log file.)", file = sys.stderr)
