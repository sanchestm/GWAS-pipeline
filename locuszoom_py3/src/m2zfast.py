...
try:
  ...
  print("Warning: gzip not available on this system", file = sys.stderr)

try:
  import bz2
except:
  print("Warning: bz2 not available on this system", file = sys.stderr)

try:
  import sqlite3
except:
  print("Error: your python interpeter is not compiled against sqlite3 (is it really out of date?)", file = sys.stderr)
  raise
...
def kill_dir(d):
  def report_error(function,path,excinfo):
    ...
    print("Error: could not remove: {path}, message was: {msg}", file = sys.stderr)
    
  if os.path.isdir(d):
    rmtree(d,onerror=report_error)
  else:
    print("Error: could not remove %s, not a directory." % d, file = sys.stderr)
...
def transSNP(snp,db_file):
  ...
  if cur.rowcount > 1:
    print("Warning: SNP %s had multiple names in the latest build.." % snp, file = sys.stderr)

  if new_name is None:
    print("Warning: tried to translate SNP %s to latest name in genome build, but it does not exist in the database table.." % str(snp), file = sys.stderr)
  elif new_name == snp:
    return snp
  else:
    print("Warning: %s is not the current name in genome build (should be: %s)" % (snp,new_name), file = sys.stderr)
    return new_name
...
class PosLookup:
  ...
  def __call__(self,snp):
    ...
    if res > 1:
      print("Warning: SNP %s has more than 1 position in database, using: %s" % (str(snp),region), file = sys.stderr)
...
def read_metal(metal_file,snp_column,pval_column,no_transform,chr,start,end,db_file,delim):
  ...
  for line in f:
    ...
    if snp in sptable:
      ...
      if is_number(pval):     
        ...
        if not pval_ok:
          print("Warning: marker %s has invalid p-value: %s, skipping.." % (snp,str(pval)), file = sys.stderr)
          continue
        ...
        print(format_str % elements, file = out)
      else:
  ...
  if invalid_pval_count > 0:
    print("Warning: of %i markers in the region, %i had invalid or missing p-values" % (marker_count,invalid_pval_count), file = sys.stderr)
  
  if found_chrpos:
    print("", file = sys.stderr)
    print(fill("WARNING: your association results file has "
                          "chr:pos SNP names. Please make sure you have selected the "
                          "correct genome build by using the --build parameter, or by "
                          "selecting the appropriate build on the website."), file = sys.stderr)
    print("", file = sys.stderr)
...
def read_epacts(epacts_file,chr,start,end,chr_col,beg_col,end_col,pval_col,no_transform):
  ...
  if epacts_file == "-":
    ...
    if not has_index:
      print( "Warning: EPACTS file was given, but could not find tabix index for it. " \
                           "EPACTS should have generated a tabix index for this file.",  file = sys.stderr)
  ...
  for line in f:
    ...
    try:
      ...
      print("Warning: skipping marker %s - could not convert chr/begin/end to integers.." % marker_name, file = sys.stderr)
      continue
...
#      continue
    ...
    if file_chrom == chr and file_begin >= start and file_end <= end:
      ...
      if is_number(pval):     
        ...
        pval_ok = all((f(dec_pval) for f in pval_checks))
        if not pval_ok:
          print("Warning: marker %s has invalid p-value: %s, skipping.." % (marker_name,str(pval)), file = sys.stderr)
          continue
        ...
        print(format_str % out_values, file = out)
      else:
        invalid_pval_count += 1

  if len(skipped_markers) > 0:
    print("Warning: skipped %i markers that did not appear to be SNPs.." % len(skipped_markers), file = sys.stderr)
...
def cleanup(files):
  ...
  for f in files:
    try:
      ...
      print( "Warning: failed to remove file %s" % f, file = sys.stderr)
...
def readWhitespaceHitList(file,db_file):
  ...
  for line in f:
    # Skip blank lines. 
    if line.strip() == "":
      print("Warning: skipping blank line in hitspec file..", file = sys.stderr)
      continue
    ...
    if e[5] == 'no'or e[5] == "":
      print("Skipping disabled line '%s' in hitspec file.." % " ".join(e), file = sys.stderr)
      continue
    ...
    if flank is not None and flank != "NA":
      if isSNP(snp):
        ...
        try:
          ...
          if fchr is None or fpos is None:
            print("Error: could not find position for SNP %s, skipping.." % e[0], file = sys.stderr)
          else:
            print("Error: bad chromosome/position for SNP %s, chr/pos were: %s,%s, skipping.." % (e[0],str(fchr),str(fpos)), file = sys.stderr)
          continue
      else:
      ...
      try:
        ...
        if isSNP(snp):
          ...
          print("Error: invalid chr/start/end in hitspec file, no chrom given, line was: '%s'" % " ".join(e), file = sys.stderr)
          continue
...
def getSettings():
  ...
  if opts.refsnp:
    ...
    if rmatches is not None:
      ...
      if alleles is not None:
        ...
        print("Warning: it appears that you specified alleles (%s) with your --refsnp, but LocusZoom "
                             "does not consider alleles, only the chrom and pos. If your results have multiple "
                             "variants at the same position, LocusZoom will not distinguish between them." % alleles, file = sys.stderr)
  ...
  if opts.multi > MULTI_CAP:
    ...
    print("Warning: --multi was higher than maximum allowed value of %i, reducing value to that limit." % MULTI_CAP, file = sys.stderr)
  ...
  if opts.ld is not None:
    ...
    if not opts.no_ld:
      ...
      if info_geno is None:
        print("Error: source %s, population %s, and build %s are not jointly supported." % (
          opts.source,opts.pop,opts.build), file = sys.stderr)
        print("See below for supported combinations.", file = sys.stderr)
        print("", file = sys.stderr)
  ...
  if opts.gwas_cat is not None:
    ...
    if gwas_cat_file is None:
      ...
      if opts.gwas_cat_file is None:
        print("Error: no gwas catalog '%s' exists for selected build, and was not a file either!" % opts.gwas_cat, file = sys.stderr)
        print("If you were trying to specify a catalog by code, they are: ", file = sys.stderr)
  ...
  if 'snpset' in m2zargs:
    print("Warning: overriding --snpset %s with option given as snpset=%s.." % (opts.snpset,m2zargs['snpset']), file = sys.stderr)
    opts.snpset = m2zargs['snpset']

  # Print warnings about deprecated options. 
  if opts.offline:
    print("Warning: --offline no longer required, option will be ignored..", file = sys.stderr)
  ...
  try:
    ...
    if ld_success:
      ...
      if os.path.isfile(ld_filename):
        print("Warning: LD file already exists for some reason: %s" % ld_filename, file = sys.stderr)
      ...
      print("Warning: LD could not be computed for SNP %s. "\
      "This SNP does not exist in the genotype files for computing LD from %s/%s/%s.." % (str(snp),source,build,pop), file = sys.stderr)
      ld_filename = None
  finally:
...
def fixUserLD(file,refsnp,db_file):
    ...
    if line.find(refsnp.snp) != -1:
      ...
      for col in (snp1_col,snp2_col):
        ...
        if chr is not None and pos is not None:
          ...
          print( "Warning: could not find position for SNP %s in user-supplied --ld file, skipping.." % str(snp),file = sys.stderr)
          skip = True
          
      if not skip:
        print( " ".join(e), file = out_file)
  ...
  if found_refsnp:
    ...
    print ("Error: user-supplied LD file does not contain the reference SNP %s.." % str(refsnp), file = sys.stderr)
    return None
...
def runQuery(query,args):
  ...
  try:
    ...
    print ("Error: SQL query failed, error was: %s" % error_msg, file = sys.stderr)
    cur = None
    file = None
  ...
  if cur is not None:
    try:
      ...
      error_msg = str(sys.exc_info()[1])
      print("Error: could not write SQL query '%s' to file. Exception was: " % error_msg, file = sys.stderr)
      file = None
...
def runQueries(chr,start,stop,snpset,build,db_file,do_annot,gene_table):
  ...
  if gene_table in db_tables:
    ...
    print("Warning: gene table '%s' not found in database, skipping gene lookups in region" % gene_table, file = sys.stderr)
  ...
  if do_annot:
    if all([x in db_tables for x in (SQLITE_SNP_POS,SQLITE_VAR_ANNOT)]):
      ...
      print("Warning: either SNP position table '%s' or annotation table '%s' were not found in database" % (SQLITE_SNP_POS,SQLITE_VAR_ANNOT), file = sys.stderr)

  if SQLITE_RECOMB_RATE in db_tables:
    results['recomb'] = runQuery(recomb_in_region,[db,SQLITE_RECOMB_RATE,chr,start,stop,build])
  else:
    print("Warning: recombination rate table '%s' not found in database, skipping recomb rate lookups in region" % SQLITE_RECOMB_RATE, file = sys.stderr)
...
def runAll(input_file,input_type,refsnp,chr,start,end,opts,args):
  ...
  if not bPocull:
    print("Error: region specified contains no SNPs, skipping..", file = sys.stderr)
    if not no_clean:
  ...
  if refsnp.pos is None:
    print ("Error: could not find position for %s, skipping.." % str(refsnp), file = sys.stderr)
    if not no_clean:
      print( "Deleting temporary files..")
      cleanup([ld_temp,metal_temp])
    return; 
  ...
  if opts.multi == 1:
    ...
    for entry in opts.snplist:
      ...
      try:
        ...
        print("Error: Tried to create temporary directory %s but failed" % temp_dir, file = sys.stderr)
        print("Current working directory is: %s" % os.getcwd(), file = sys.stderr)
        raise
      ...
      try:
        ...
        print(fill("Error: A sqlite3 disk error was caught - this is *usually* the "
                              "result of /tmp or /var/tmp being filled to capacity on the "
                              "machine, or a permissions error on the database file itself. ",80),file = sys.stderr)
        
        print("", file = sys.stderr)

        print(fill("You should check that /tmp or /var/tmp are not filled to "
                              ...
                              "not full. ",80), file = sys.stderr)

        print("", file = sys.stderr)

        print("For reference, your database file is currently located here: %s\n" % opts.sqlite_db_file, file = sys.stderr)

        print("The original error was:", file = sys.stderr)
        raise
        ...
        if len(image_file) == 0:
          ...
          for i in xrange(len(image_file)):
            ...
            try:
              ...
              print("Error: extracting image %s failed.." % image, file = sys.stderr)
