...
def ld_rsquare_indexsnp_vcf(index_pos,vcf_file,region,tabix_path="tabix",ignore_filter=False):
  ...
  if not int(index_pos) >= 0:
    print("Error computing LD: index SNP position %s is invalid.." % str(index_pos), file = sys.stderr)
    return
  ...
  if stdout == '':
    print("Error: while calculating LD from VCF file: index SNP position %s does not exist in file.. " % str(index_pos), file = sys.stderr)
    return

  if stderr != '':
    print("Error: while calculating LD from VCF file: tabix generated an error: \n%s" % stderr, file = sys.stderr)
    return
  ...
  with open(ld_filename,'w') as out:
    print ("\t".join(['snp1','snp2','dprime','rsquare']), file = out)
...
#        continue
      ...
      if seen.get((chr,pos)) is not None:
        print ("Warning: multiple variants at same position (%s) in VCF file, using the first variant" % chrpos, file = out)
        continue
      else:
        seen[(chr,pos)] = 1
      ...
      print("\t".join([chrpos,index_chrpos,"NA",str(rsq)]), file = out)

      markers += 1

  if markers == 0:
    print("Error: no valid markers in VCF file to compute LD from..", file = sys.stderr)
    return
...
def ld_dprime_indexsnp_vcf(index_pos,vcf_file,region,tabix_path="tabix",ignore_filter=False):
  ...
  if len(index_ref) != 1:
    print("Error: while calculating LD from VCF file: index SNP is not a SNP - ref allele was %s, alt allele was %s" % (index_ref,index_alt), file = sys.stderr)
    return
...
#        continue
      ...
      # Have we seen this variant already? 
      if seen.get((chr,pos)) is not None:
        print("Warning: multiple variants at same position (%s) in VCF file, using the last variant" % chrpos, file = sys.stderr)
      else:
        seen[(chr,pos)] = 1
      ...
      # Write out in the format expected by locuszoom. 
      print( "\t".join([chrpos,index_chrpos,str(d_prime),"NA"]), file = out)
