...
* phenotype statistical descriptions file: [here](https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/PATHNAMEdata_distributions.html) 
...
Quantile normalized separately for males and females then regress out the effect of other covariates (simulataneously) if they are significant and explain more than REGRESSTHRS of variance. 
...
df= df[c('trait', 'n','heritability',   'heritability_se', 'pval')]
pretify(df)
...
eqtl = ""
if (file.exists("phewas/pretty_eqtl_table.tsv")) {
...
sqtl = ""
if (file.exists("phewas/pretty_sqtl_table.tsv")) {
...
* maf: frequency of the TopSNP in the rats used for this study
...
qtls_v2 = qtls %>% rename( 'TopSNP' = 'SNP','maf'= 'Freq', 'betase' = 'se')
pretify(  qtls_v2)
...
for( i in rownames(qtls) ){
   ...
   tryCatch(
    expr = {
       ...
       tempcausal = ann %>% filter(SNP_qtl == qtls[i, 'SNP']) %>% filter(trait == qtls[i, 'trait'])  %>% select(c('SNP','Freq','b','p','R2','DP','annotation','putative_impact','gene','HGVS.c','HGVS.p')) %>% distinct()
   ...
   if (tolower(eqtl) != "") {
       eqtltemp = eqtl %>% filter(SNP == paste0('chr',qtls[i, 'SNP'])) %>% filter(trait == qtls[i, 'trait'])  %>% select(c('SNP_eqtldb', '-Log10(p)_eqtldb', 'tissue', 'R2', 'DP', 'Ensembl_gene', 'slope', 'af', 'slope'))%>% rename( 'gene' = 'Ensembl_gene') %>% distinct()  # 'presence_samples'
   ...
   if (tolower(sqtl) != "") {
       sqtltemp = sqtl %>% filter(SNP == paste0('chr',qtls[i, 'SNP'])) %>% filter(trait == qtls[i, 'trait'])  %>% select(c('SNP_sqtldb', '-Log10(p)_sqtldb', 'tissue','R2', 'DP', 'Ensembl_gene',  'slope', 'af', 'slope')) %>% rename( 'gene' = 'Ensembl_gene') %>% distinct()   #'presence_samples',
